    ...
    <table border=1 style="margin-top:30px">
      ...
      <th>Location</th>-->
        ...
        <td><?php echo $row["ATMShortName"]; ?></td>-->
                ...
                } else if (strpos($row["Panel_make"], 'comfort_sbitom2') !== FALSE) {
                    $sql1 = "select SensorName as Description,Camera from comfort_sbitom2 where (ZONE='" . $row["zone"] . "' and SCODE='" . $row['alarm'] . "')";
                } 



                else if (strpos($row["Panel_make"], 'comfort') !== FALSE) {
                    $sql1 = "select SensorName as Description,Camera from comfort where (ZONE='" . $row["zone"] . "' and SCODE='" . $row['alarm'] . "')";
                } 


                else if (strpos($row["Panel_make"], 'rass_pnb') !== FALSE) {
                    $sql1 = "select SensorName as Description,Camera from rass_pnb where (ZONE='" . $row["zone"] . "' and SCODE='" . $row['alarm'] . "')";
                } 


                else if (strpos($row["Panel_make"], 'smarti_pnb') !== FALSE) {
                    $sql1 = "select SensorName as Description,Camera from smarti_pnb where (ZONE='" . $row["zone"] . "' and SCODE='" . $row['alarm'] . "')";
                } 
                else {
