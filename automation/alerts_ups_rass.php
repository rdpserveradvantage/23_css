...
SELECT b.id,b.panelid,b.seqno,b.zone,b.alarm,b.createtime,b.receivedtime,b.comment,b.status,b.sendtoclient,b.closedBy,b.closedtime,b.sendip,b.alerttype,b.location,b.priority,b.AlertUserStatus  FROM `sites` a,`backalerts` b WHERE (a.OldPanelID=b.panelid or a.NewPanelID=b.panelid) AND (a.Panel_Make='RASS' OR a.Panel_Make = 'rass_boi' OR a.Panel_Make = 'rass_pnb' OR a.Panel_Make='rass_sbi') AND (b.alarm IN ('AT','AR') AND b.zone IN ('029','030')) AND CAST(b.receivedtime AS DATE)= '".$yesterday."'
";
	$_sql = mysqli_query($con,$_sql_qry); 
	
	
	$abc = "SELECT  a.Customer,a.Bank,a.ATMID,a.ATMShortName,a.SiteAddress,a.DVRIP,a.Panel_make,a.zone as zon,a.City,a.State,b.id,b.panelid,b.createtime,b.receivedtime,b.comment,b.zone,b.alarm,b.closedBy,b.closedtime FROM sites a,`alerts_acup_new` b WHERE (a.OldPanelID=b.panelid or a.NewPanelID=b.panelid) and b.zone IN ('029','030') and b.alarm='AT' ";
...
$objPHPExcel->getProperties()->setCreator("Dhaval")
    ...
    ->setTitle("Daily UPS Report RASS")
    ->setSubject("Report")
...
while ($res = mysqli_fetch_array($result)) {
	...
	if ($res["alarm"] == "AT" and $res["zone"] == "029") {
		...
		$xyz = "select createtime from alerts_acup_new where panelid='" . $_panel_id . "' and zone='029' and alarm='AR' and createtime>'" . $timestamp . "' order by createtime limit 1";
	...
	if ($res["alarm"] == "AT" and $res["zone"] == "030") {
		...
		$xyz1 = "select createtime from alerts_acup_new where panelid='" . $_panel_id . "' and zone='030' and alarm='AR' and createtime>'" . $timestamp . "' order by createtime limit 1";
...
// Define file path
$folder = 'exports/';
$filename = "rass_report_{$date}.xlsx";
