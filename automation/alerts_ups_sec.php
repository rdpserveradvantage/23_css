...
SELECT b.id,b.panelid,b.seqno,b.zone,b.alarm,b.createtime,b.receivedtime,b.comment,b.status,b.sendtoclient,b.closedBy,b.closedtime,b.sendip,b.alerttype,b.location,b.priority,b.AlertUserStatus FROM `sites` a,`backalerts` b WHERE (a.OldPanelID=b.panelid or a.NewPanelID=b.panelid) AND a.Panel_Make = 'SEC' AND (b.alarm IN ('BA','BR') AND b.zone IN ('027','028')) AND CAST(b.receivedtime AS DATE)= '".$yesterday."'
";    
	$_sql = mysqli_query($con,$_sql_qry); 
	
	
	$abc = "SELECT  a.Customer,a.Bank,a.ATMID,a.ATMShortName,a.SiteAddress,a.DVRIP,a.Panel_make,a.zone as zon,a.City,a.State,b.id,b.panelid,b.createtime,b.receivedtime,b.comment,b.zone,b.alarm,b.closedBy,b.closedtime FROM sites a,`alerts_acup_new` b WHERE (a.OldPanelID=b.panelid or a.NewPanelID=b.panelid) and b.zone IN ('027','028')and a.Panel_Make = 'SEC' and b.alarm ='BA'";
...
while ($res = mysqli_fetch_array($result)) {
	...
	if ($res["alarm"] == "BA" and $res["zone"] == "027") {
		...
		$xyz = "select createtime from alerts_acup_new where panelid='" . $_panel_id . "' and zone='027' and alarm='BR' and createtime>'" . $timestamp . "' order by createtime limit 1";
	...
	if ($res["alarm"] == "BA" and $res["zone"] == "028") {
		$UPS_Power_Failure_Alert_Received_Date = $date;
		$UPS_Power_Failure_Alert_Received_Time = $time;
		
		$xyz1 = "select createtime from alerts_acup_new where panelid='" . $_panel_id . "' and zone='028' and alarm='BR' and createtime>'" . $timestamp . "' order by createtime limit 1";
...
// Define file path
$folder = 'exports/';
$filename = "sec_report_{$date}.xlsx";
