...
// --- Main SQL query ---
  ...
  AND (
         (b.zone = '024' AND a.Panel_make IN ('RASS','SMART-IN'))
      OR (b.zone = '022' AND a.Panel_make IN ('SMART -I','SEC'))
      OR (b.zone IN ('032','033') AND a.Panel_make = 'securico_gx4816')
      )";
       ...
       b.zone, b.alarm, b.closedBy, b.closedtime, b.sendip, b.sip2
FROM sites a
JOIN alerts_backup b
    ON a.NewPanelID = b.panelid
WHERE (b.receivedtime between '" . $yesterday . " 00:00:00' and '" . $yesterday . " 23:59:59')
  AND (
         (b.zone = '024' AND a.Panel_make IN ('RASS','SMART-IN'))
      OR (b.zone = '022' AND a.Panel_make IN ('SMART -I','SEC'))
      OR (b.zone IN ('032','033') AND a.Panel_make = 'securico_gx4816')
		...
		$backalerts_old = "SELECT a.Customer, a.Bank, a.ATMID, a.ATMShortName, a.SiteAddress, a.DVRIP,
				...
				b.zone, b.alarm, b.closedBy, b.closedtime, b.sendip, b.sip2
		FROM sites a
		JOIN backalerts_backup b ON a.OldPanelID = b.panelid
        WHERE (b.receivedtime between '" . $yesterday . " 00:00:00' and '" . $yesterday . " 23:59:59')
  AND (
         (b.zone = '024' AND a.Panel_make IN ('RASS','SMART-IN'))
      OR (b.zone = '022' AND a.Panel_make IN ('SMART -I','SEC'))
      OR (b.zone IN ('032','033') AND a.Panel_make = 'securico_gx4816')
		...
		$backalerts_new = "SELECT a.Customer, a.Bank, a.ATMID, a.ATMShortName, a.SiteAddress, a.DVRIP,
				...
				b.zone, b.alarm, b.closedBy, b.closedtime, b.sendip, b.sip2
		FROM sites a
		JOIN backalerts_backup b ON a.NewPanelID = b.panelid
        WHERE (b.receivedtime between '" . $yesterday . " 00:00:00' and '" . $yesterday . " 23:59:59')
  AND (
         (b.zone = '024' AND a.Panel_make IN ('RASS','SMART-IN'))
      OR (b.zone = '022' AND a.Panel_make IN ('SMART -I','SEC'))
