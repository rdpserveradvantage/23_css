...
SELECT b.id,b.panelid,b.seqno,b.zone,b.alarm,b.createtime,b.receivedtime,b.comment,b.status,b.sendtoclient,b.closedBy,b.closedtime,b.sendip,b.alerttype,b.location,b.priority,b.AlertUserStatus FROM `backalerts` b,sites a WHERE (a.OldPanelID=b.panelid or a.NewPanelID=b.panelid) AND (a.Panel_Make='SMART -I' OR a.Panel_Make ='SMART -IN' OR a.Panel_Make ='smarti_boi' OR a.Panel_Make ='smarti_pnb') AND (b.alarm IN ('BA','BR') AND b.zone IN ('001','002')) AND CAST(b.receivedtime AS DATE)= '".$yesterday."' ";
...
$objPHPExcel->getProperties()->setCreator("Dhaval")
    ...
    ->setTitle("Daily UPS Report SMARTI")
    ->setSubject("Report")
...
while ($res = mysqli_fetch_array($result)) {
	...
	if ($res["alarm"] == "BA" and $res["zone"] == "001") {
		...
		$xyz = "select createtime from alerts_acup_new where panelid='" . $_panel_id . "' and zone='001' and alarm='BR' and createtime>'" . $timestamp . "' order by createtime limit 1";
	...
	if ($res["alarm"] == "BA" and $row["zone"] == "002") {
		$UPS_Power_Failure_Alert_Received_Date = $date;
		$UPS_Power_Failure_Alert_Received_Time = $time;
		
		$xyz1 = "select createtime from alerts_acup_new where panelid='" . $_panel_id . "' and zone='002' and alarm='BR' and createtime>'" . $timestamp . "' order by createtime limit 1";
...
// Define file path
$folder = 'exports/';
$filename = "smarti_report_{$date}.xlsx";
