...
$insertqrydate = date("Y-m-d H:i:s");
...
// echo 
$sql = "update sites set Status='$Status',Phase='$Phase',Customer='$Customer',Bank='$Bank',ATMID='$ATMID',ATMID_2='$ATMID_2',ATMID_3='$ATMID_3',ATMID_4='$ATMID_4',ATMShortName='$ATMShortName',SiteAddress='$SiteAddress',
TrackerNo='$TrackerNo',City='$City',State='$fetch[0]',Zone='$Zone',Panel_Make='$Panel_Make',OldPanelID='$OldPanelID',
NewPanelID='$NewPanelID',DVRIP='$DVRIP',DVRName='$DVRName',UserName='$UserName',Password='$Password',live='$live',
eng_name='$engname',editby='" . $_SESSION['name'] . "',site_remark='" . $remark . "',PanelIP='" . $PanelsIP . "',DVR_Model_num='" . $DVR_Model_num . "',Router_Model_num='" . $Router_Model_num . "',RouterIp='" . $RouterIp . "',old_atmid='" . $old_atmid . "',panel_power_connection='" . $power_connection . "',live_date='" . $_liveDate . "' where SN='$sn'";
...
if ($result = mysqli_query($conn, $sql)) {
    ...
    $insertqrylogs = mysqli_query($con, "insert into query_logs(query,created_at) values ('" . $sql . "','" . $insertqrydate . "')  ");

    $alldvrlive = mysqli_query($con, "insert into all_dvr_live (`UserName`, `Password`, `dvrname`, `IPAddress`, `port`, `customer`,  `atmid`, `SN`, `project`, `ipcamport`, `ipcamtype`) 
			values ('$UserName','$Password','$DVRName','$DVRIP','$port','$Customer','$ATMID',$sn','sites','$port','$DVRName') ");

    echo $qDvr_health = "update dvr_health set ip='" . $DVRIP . "' , dvrtype='" . $DVRName . "',live='" . $live . "',atmid='" . $ATMID . "' where atmid='" . $ATMID . "' ";
    if (mysqli_query($conn, $qDvr_health)) {
    } else {
        echo 'dvr_health';
    }

    $qPanel_health = "update panel_health set ip='" . $PanelsIP . "' , panelName='" . $Panel_Make . "',panelid='" . $NewPanelID . "',atmid='" . $ATMID . "' where atmid='" . $ATMID . "' ";
    if (mysqli_query($conn, $qPanel_health)) {
    } else {
        echo 'panel_health';
    }

    $sql2 = "insert into  site_attachment(site_id,files,installation_dt)values('" . $sn . "','" . $imgdir . "','" . $insdates . ' ' . $t . "')";
    if ($runsql2 = mysqli_query($conn, $sql2)) {
    } else {
        echo 'site_attachment';
    }
    ...
    if ($gnum > 0) {
        ...
        if ($gsmupdaterun) {
            $insertqrylogs = mysqli_query($con, "insert into query_logs(query,created_at) values ('" . $gsmupdate . "','" . $insertqrydate . "')  ");
        }
    } else {
        ...
        if ($gsminsrun) {
            $insertqrylogs = mysqli_query($con, "insert into query_logs(query,created_at) values ('" . $gsmins . "','" . $insertqrydate . "')  ");
        }
    }
    ...
    if ($circle != '' && $zonal != '' && $site_type != '') {
        ...
        $process_site_circle_query =  mysqli_query($conn, $site_circle_query);
        if ($process_site_circle_query) {
            $insertqrylogs = mysqli_query($con, "insert into query_logs(query,created_at) values ('" . $site_circle_query . "','" . $insertqrydate . "')  ");
        }
    }
