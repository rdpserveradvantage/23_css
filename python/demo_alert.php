...
WHERE is_viewalertReportCreated = 0 ORDER BY id DESC LIMIT 1");
...
while ($getsql_result = mysqli_fetch_assoc($getsql)) {
    ...
    $recordid  = $getsql_result['id'];

    $reportdate = str_replace('-', '', $alerts_data_date);

echo 
$sql = "SELECT  a.Customer,a.Bank,a.ATMID,a.ATMShortName,a.SiteAddress,a.DVRIP,a.Panel_make,a.zone as zon,a.City,a.State,b.id,b.panelid,b.createtime,b.receivedtime,b.comment,b.zone,b.alarm,b.closedBy,b.closedtime,b.sendip FROM sites a,`$newtableName` b WHERE (a.OldPanelID=b.panelid or a.NewPanelID=b.panelid)  and b.receivedtime between '$alerts_data_date 00:00:00' and '$alerts_data_date 23:59:59' order by receivedtime desc";

echo 
$sql2 = "SELECT  a.Customer,a.Bank,a.ATMID,a.ATMShortName,a.SiteAddress,a.DVRIP,a.Panel_make,a.zone as zon,a.City,a.State,b.id,b.panelid,b.createtime,b.receivedtime,b.comment,b.zone,b.alarm,b.closedBy,b.closedtime,b.sendip FROM sites a,`back$newtableName` b WHERE (a.OldPanelID=b.panelid or a.NewPanelID=b.panelid)  and b.receivedtime between '$alerts_data_date 00:00:00' and '$alerts_data_date 23:59:59' order by receivedtime desc";
...
return ; 
    ...
    header('Content-Disposition: attachment; filename="exportalertReport_'.$reportdate.'.csv"');
    ...
    while ($row_new = mysqli_fetch_array($result_new)) {
        ...
        } else if ($row_new["Panel_make"] == "comfort_diebold") {
            $sql1 = "select SensorName as Description,Camera from comfort_diebold where zone='" . $row_new["zone"] . "' ";
        }
    ...
    mysqli_query($con,"update alerts_data_tracker_datewise set is_viewalertReportCreated=1 where id='".$recordid."'");
