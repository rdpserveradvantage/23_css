...
Values('" . $SN . "','','" . $uploadedImagePath . "','" . $datetime . "')");
	...
	if (!$result10server) {
		$sitesqry = "update sites set modify=2 where ATMID='$ATMID'";
		$upd_sites = mysqli_query($conn, "update sites set modify=2 where ATMID='$ATMID'");
		if ($upd_sites) {
			$insertqrylogs = mysqli_query($con, "insert into query_logs(query,created_at) values ('" . $sitesqry . "','" . $insertqrydate . "')  ");
		}
	}
